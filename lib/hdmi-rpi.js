...
function init_listeners() {
	...
	update.on('status.power.active', (data) => { command(data.new); });
...
function check() {
	...
	switch (status.vehicle.ignition) {
		case 'off' : correct_power_state = true; break;
			...
			switch (status.hdmi.rpi.power) {
				case false : {
					...
					switch (status.hdmi.rpi.power_override) {
						...
						case true : {
							log.msg('Power state incorrect, but currently overridden, Correcting');
							hdmi_rpi.hdmi_client.power(correct_power_state);
							return false;
						}
					}
